...
func ValidationError(w http.ResponseWriter, errs map[string][]string) {
	var invalids []domain.InvalidParam

	for field, msg := range errs {
		invalids = append(invalids, domain.InvalidParam{
			Name:   field,
			Reason: msg[0],
		})
	}

	p := domain.NewProblem("Validation failed.")
	p.InvalidParams = invalids

	w.WriteHeader(ErrorStatusCode(domain.EINVALID))
...
func Error(w http.ResponseWriter, r *http.Request, err error) {
	...
	json.NewEncoder(w).Encode(domain.NewProblem(message))
}
