...
func (fq *FeedQuery) QueryCollections() *CollectionQuery {
	query := &CollectionQuery{config: fq.config}
...
// Returns a *NotSingularError when exactly one Feed entity is not found.
...
// Returns a *NotSingularError when exactly one Feed ID is not found.
...
func (fq *FeedQuery) WithCollections(opts ...func(*CollectionQuery)) *FeedQuery {
	query := &CollectionQuery{config: fq.config}
...
func (fq *FeedQuery) sqlAll(ctx context.Context) ([]*Feed, error) {
	...
	if query := fq.withCollections; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Feed, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(feed.CollectionsPrimaryKey[1], fks...))
			},
