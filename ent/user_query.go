...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCollections; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Collections = []*Collection{}
		}
		query.withFKs = true
		query.Where(predicate.Collection(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CollectionsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_collections" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_collections" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Collections = append(node.Edges.Collections, n)
		}
	}
