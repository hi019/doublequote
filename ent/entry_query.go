...
func (eq *EntryQuery) QueryFeed() *FeedQuery {
	query := &FeedQuery{config: eq.config}
...
func (eq *EntryQuery) QueryCollectionEntries() *CollectionEntryQuery {
	query := &CollectionEntryQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EntryQuery) Clone() *EntryQuery {
	...
	return &EntryQuery{
		...
		withFeed:              eq.withFeed.Clone(),
		withCollectionEntries: eq.withCollectionEntries.Clone(),
...
func (eq *EntryQuery) WithFeed(opts ...func(*FeedQuery)) *EntryQuery {
	query := &FeedQuery{config: eq.config}
...
func (eq *EntryQuery) WithCollectionEntries(opts ...func(*CollectionEntryQuery)) *EntryQuery {
	query := &CollectionEntryQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withCollectionEntries = query
	return eq
}
...
func (eq *EntryQuery) sqlAll(ctx context.Context) ([]*Entry, error) {
	...
	if query := eq.withFeed; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Entry)
		for i := range nodes {
			if nodes[i].feed_entries == nil {
				continue
			}
			fk := *nodes[i].feed_entries
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "feed_entries" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := eq.withCollectionEntries; query != nil {
		...
		query.Where(predicate.CollectionEntry(func(s *sql.Selector) {
			s.Where(sql.InValues(entry.CollectionEntriesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "entry_id" returned %v for node %v`, fk, n.ID)
			}
