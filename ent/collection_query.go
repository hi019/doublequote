...
func (cq *CollectionQuery) QueryFeeds() *FeedQuery {
	query := &FeedQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CollectionQuery) WithFeeds(opts ...func(*FeedQuery)) *CollectionQuery {
	query := &FeedQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CollectionQuery) sqlAll(ctx context.Context) ([]*Collection, error) {
	...
	if query := cq.withUser; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_collections" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withFeeds; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Collection, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Feeds = []*Feed{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(collection.FeedsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "feeds": %w`, err)
		}
		query.Where(feed.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "feeds" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Feeds = append(nodes[i].Edges.Feeds, n)
			}
		}
	}
